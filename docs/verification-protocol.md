...
- Sequence per challenge, after the forwarded context: one length estimate, one timing estimate, then one token estimate per position, the interlock answering each non-terminal token estimate with a reveal.
