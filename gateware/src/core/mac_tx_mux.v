// mac_tx_mux — frame-boundary mux of 3 MAC-TX-client sources onto one MAC-TX.
// in0 = forwarded traffic (eth_reframe), in1 = request cert frame, in2 = response
// cert frame. A whole frame from the selected source passes through (sof..eof)
// before re-arbitration, so frames are never interleaved. Cert frames get
// PRIORITY (they're rare, ~1/window, and must not be starved by sustained
// forwarded traffic); forwarded traffic waits at most one frame. Plain Verilog.
...
module mac_tx_mux (
    ...
    // arbitrate when idle: certs (1,2) before forwarded traffic (0)
    wire       any  = in0_rdy | in1_rdy | in2_rdy;
    wire [1:0] pick = in1_rdy ? 2'd1 : in2_rdy ? 2'd2 : 2'd0;

    wire        s_rdy = (sel==2'd0) ? in0_rdy : (sel==2'd1) ? in1_rdy : in2_rdy;
    wire        s_sof = (sel==2'd0) ? in0_sof : (sel==2'd1) ? in1_sof : in2_sof;
    wire        s_eof = (sel==2'd0) ? in0_eof : (sel==2'd1) ? in1_eof : in2_eof;
    wire [31:0] s_dat = (sel==2'd0) ? in0_dat : (sel==2'd1) ? in1_dat : in2_dat;
    wire [1:0]  s_bv  = (sel==2'd0) ? in0_bv  : (sel==2'd1) ? in1_bv  : in2_bv;
